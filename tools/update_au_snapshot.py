"""Capture AU FD page HTML in a real browser session.

This helper opens a visible Chromium window (Playwright headful mode), so you
can solve any Cloudflare challenge manually. After you confirm the FD table is
...
DEFAULT_URL = "https://www.au.bank.in/interest-rates/fixed-deposit-interest-rates"
DEFAULT_OUT = Path("snapshots/au_fd_interest_rates.html")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Update AU FD local snapshot using headful browser automation."
    )
    p.add_argument("--url", default=DEFAULT_URL, help="Target AU page URL")
    p.add_argument(
        "--out",
        default=str(DEFAULT_OUT),
        help="Output HTML path (default: snapshots/au_fd_interest_rates.html)",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        print("2) Ensure the FD rates table is visible")
